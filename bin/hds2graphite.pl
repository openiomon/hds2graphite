...
sub registerservice {
	...
	if($storagename eq 'ALL') {
		...
		if(!$realtime) {	
			...
			if(-f $servicefile) {
				console("There is allready a service with the name hds2graphite-".$storagename." registerd. You can either start, stop or restart the service. For updates to servicefile please deregister an register again.");
			} else {
			...
			console("Registering realtime service for ".$storagename." (Type: ".$storage{$storagename}{'type'}." / S/N: ".$storage{$storagename}{'serial'}.")");
                        ...
                        if(-f $servicefile) {
                                console("There is allready a service with the name hds2graphite-rt-".$storagename." registerd. You can either start, stop or restart the service. For updates to servicefile please deregister an register again.");
                        } else {
