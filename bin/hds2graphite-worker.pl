...
my $logfile = ""; # Logfile definition => Kommt aus Konfig-Datei
my $etlogfile = ""; # Definition der Logfile-Location für das Exporttool => Kommt aus Konfig-Datei

my $conf = ""; #Lokation des Script-Configuration-Datei => wird an Script übergeben
my $serial = ""; # Seriennummer des Speichersystems welches gerade importiert wird => Kommt aus Konfig-Datei
my $exportpath = ""; # Verzeichnis in dem die Export Tool Daten liegen welche importiert werden sollen => Kommt aus Konfig-Datei bzw. wird ggf. übergeben
my $exporttoolretrycnt = 3; #defines retry count for exporttool
my $exporttoolretrytimeout = 2; #defines retry timeout for exportool in minutes
my $horcminst = ""; # Instanz-Nummer welche zum Speichersystem zeigt das gerade importiert wird. => Kommt aus Konfig-Datei
my $metricconf = ""; # Konfigurationsdatei welche die zu importierenden Metrics enthält => Kommt aus Konfig-Datei
my $storagename = ""; # Name des Speichersystems das gerade importiert wird => Wird übergeben.
my $noexporttool = false; # Schalter zum aktivieren oder deaktivieren des Export Tool => Wird zur laufzeit gesetzt.
my $hourstoimport = 1; # Anzahl der Stunden die rückwirkend importiert werden sollen. => Wird an das Script übergeben.
my $exporttoolstatus = true;

my $graphitehost = ""; # IP des Graphite hosts => Kommt aus Konfig-Datei
my $graphiteport = ""; # Port des Graphite hosts => Kommt aus Konfig-Datei

my $cciuser = ""; # CCI user => Kommt aus Konfig-Datei
my $ccipasswd = ""; # CCI passwd => Kommt aus Konfig-Datei

my $usevirtualldev = false; #Flag to specify whether virtual LDEV IDs should be used for storing data of GAD volumes.

my $exporttoolpath = ""; # Verzeichnis im dem das Export Tool liegt welches zu dem Speichersystemtyp passt das gerade importiert wird. => Kommt aus Konfig-Datei

my %namereference; # Hash mit der Zuordnung zwischen Speichersystemnamen und Seriennummern => Kommt aus Konfig-Datei und wird zur Laufzeit gefüllt.
my %arraytype; # Hash mit der Zuordnung zwischen Speichersystemtype und Seriennummern => Kommt aus Konfig-Datei und wird zur Laufzeit gefüllt.
my %cciinstances; # Hash mit der Zuordnung zwischen CCI/HORCM Instanz und Seriennummer => Kommt aus Konfig-Datei und wird zur Laufzeit gefüllt.
my %ccicredentials; # Hash including all CCI credentials for the storage arrays;
my %metricfiles; # Hast mit allen Mertic-Files für alle Speichersystemtypen => Kommt aus Konfig-Datei und wird zur Laufzeit gefüllt.
my %exporttooltemplates; # Hash mit allen Exporttool Template File für die einen Arrayrypen => Kommt auf der Konfig-Datei und wird zur Laufzeit gefüllt.
my %exporttoolparams; # Hash containing export tool params for all serial numbers-
my %serialreference; # Reverse-Hash für die Zuorning zwischen Seriennummer und Speichersystemnamen => Kommt aus Konfig-Datei und wird zur Laufzeit gefüllt.
my %vsms; # Refernce hash for GAD VSM.
my %graphiteconf;
my %serviceconf;

my %config; # Hast der alle Metriken enthät die aktuell importiert werden sollen. => Kommt aus Konfig-Datei und wird zur Laufzeit gefüllt.

my %poolstats; # Hash welche Pool Statistiken für das Speichersystem beinhaltet => Wird zur Laufzeit gefüllt.
my @pgs; # Array welche alle Parity Groups eines Speichersystems beinhaltet => Wird zur Laufzeit gefüllt.

my %ldevmapping; # Hash der alle Zuordnungen zwischen LDEVs und HDP Pools oder PGs beinhaltet. => Wird zur Laufzeit gefüllt.
my %lumapping; # Hash der alle Zuordnuningen zwischen LUs und LDEVs beinhaltet. => Wird zur Laufzeit gefüllt.
my %hsdreference; # Hash der die Zuordnung zwischen HSD Namen und HSD-Index enthät. => Wird zur Laufzeit gefüllt.

my %systemips; # Hash der alle IPs der Systeme beinhaltet. Bei VSP und G1x00 SVP IP bei beide Gx00 Controller IPs. => Wird zur Laufzeit auf der Konfigurationsdatei gefüllt.
my $starttime = time;
...
sub printUsage {
    ...
    print("   -exportpath <dir>        points to directory where exporttool data is stored for optional manual import\n");
...
sub parseCmdArgs {
	...
	if($storagename eq "") {
		...
		# get storage system serial and CCI instance out of the configdata
	...
	# the the -exportpath option is specified the data stored in this directory will be imported. There will be no run of export tool itself
	...
	# before starting setting the needed defaults if specified per Storage-System
	#
...
# Sub will check if system is Panama 
...
# Sub will check if system is Panama2
...
# Sub to read the metric configuration file for each tpy of storage.
    ...
    while(<$configfp>) {
        ...
        if(substr($confline,0,1) ne "#") {
            ...
            if($perffile =~ '\*') {
				...
				# Line containing § (paragraph) indicate that a part of the filename will be used to specify the ITEM (MP Board, etc.) for with the data is conained in this CSV file. There is for example one CSV file per MP instand of one file
				# containing all MP data
            } elsif ($perffile =~ "%") {
                ...
                foreach my $result (@results) {
					...
					#print "From: ".$from." to ".$to." of line: ".$lsline[8]."\n";
					#print "Item in Readmetric: ".$item."\n";
                    $config{$lsline[8]}{"table"} = $table;
                    $config{$lsline[8]}{"metric"} = $metric;
...
sub importmetric {
    ...
    foreach my $filename (sort keys (%config)) {	
		if(!-f $filename) {
			$log->warn("The file ".$filename." doesn't exist also it is defined in the metrics configuration file. Check either export tool or remove file from metrics configuration file!");
			next;
		}	
		my $metricstatcnt = 0;
        ...
        while(<$inputfp>) {
            ...
            if ($linecnt == 2) {
				...
				# Read the header line from the metric csv file that is currently imported this header line might contain LDEVs, Ports, etc.! Since the amount if colums in the metric file is not unlimited there could be multiple
				# header line in one metric csv file which will be followed by data line belonging to the items specified by the header.
                my @values = split (",",$line);
                splice(@items);
                my $num_elements = scalar @values;
                if($config{$filename}{"itemselect"} eq "auto") {
		    		# Die Items werden bei auto anhand des Headers erzeugt.
                    for(my $i=2;$i<$num_elements;$i++) {
                        my $item = $values[$i];
						# Da graphite nicht mit Leerzeichen, Zeilenumbrücken, Anführungszeichen und Punkten zurecht kommt müssen diese entfernt bzw. ersetzt werden.
                        $item =~s/\"//g;
                        $item =~s/\./_/g;
                        $item =~s/\s/_/g;
                        $item =~s/\r//g;
						...
						# Die LDEVs beinhalten in der Exporttool-Dateien immer zusätze wie x, e usw... Diese sollte entfernt werden...
						if($table eq "LDEV") {
							$item = substr($item,0,8);
						}
						# Bei Remote Copy werden die Klammern und Einheiten in den Item-Namen entfernt da diese in Graphite nicht benötigt werden.
						if($table eq "RC") {
		    		...
		    		# If the ITEM is not specified in the CSV File but comes from the configuration file we need to hat this item to our ITEM-array
					if($table eq "CACHE") {
						for(my $i=2;$i<$num_elements;$i++) {
							my $item = $values[$i];
							# Da graphite nicht mit Leerzeichen, Zeilenumbrücken, Anführungszeichen und Punkten zurecht kommt müssen diese entfernt bzw. ersetzt werden.
				...
				# Now we are looping through the colums of each line so we can read the data and allign it to the item (e.g. LDEV-ID, Port, MP, etc.)
                for(my $i=2;$i<$num_elements;$i++) {
                    if ($values[$i] >= 0) {
                        my $value = $values[$i];
                        $value =~ s/\r//g;
						if($table eq "LU") {
                        	...
                        	my $port = "";
								...
								# VSP is giving back LUN-ID in HEX using RAIDCOM while all other systems are giving LUN-IDs back in DEC
								$hsdindex = sprintf("%02s",hex($hsdindex));
								my $ldev = "";
		
								# The skip variable is used to skip LDEVs where not information from the RAIDCOM data is available.
								# This might happen if export tool data contains performance metrics for LDEVS that have been deleted during the monitoring intervall.
								...
								if($type eq "vsp") {
									if(defined $lumapping{$port}{$hsdindex}{sprintf('%03s',hex($lu))}) {
										...
										$log->trace("No LDEV-ID found for LU: ".$lu." in HSD: ".$hsdindex." on port: ".$port." for ".$namereference{$serial}."! Please check raidcom since exporttool is reporting this LDEV!");
										$skip = true;
									}
                                } else {
								...
								if(!$skip) {
									...
									if (defined($ldevmapping{$ldev}{"rsgid"})) {
										...
										if($rsgid != 0) {
											#print "Found RSGID = ".$rsgid." for LDEV: ".$ldev."\n";
							...
							# Since there is no information about PG or DP on the export tool data this information will be added from the hashed filled wit h RAIDCOM query information.
							my $ldev = $items[$i-2];
							if(!defined $ldevmapping{$ldev}{"entity"}) {
								...
								if (defined($ldevmapping{$ldev}{"rsgid"})) {
									my $rsgid = $ldevmapping{$ldev}{"rsgid"};
									if($rsgid != 0) {
										#print "Found RSGID = ".$rsgid." for LDEV: ".$ldev." Virtual LDEV: ".$ldevmapping{$ldev}{"virtualldev"}."\n";
										if($ldevmapping{$ldev}{"virtualldev"} eq "00:FF:FF") {
...
# Since the command that need to be executed is diffrent depending of the storage system typ. It is necessary to handle every array type separat.
...
sub startexporttool {
	...
	if(!$pid) {
		...
		if (($arraytype{$serial} eq "g1000")||($arraytype{$serial} eq "g1500")||isPanama($arraytype{$serial}) || isPanama2($arraytype{$serial})) {
			...
			# Die Variale $javacmd wird nur benötigt um den Aufruf in das Log zu schreiben...
		...
		# Das Export Tool ist nicht sauber durchgelaufen...
	...
	while(true) {
		...
		alive();
		if ($res == -1) {
			$log->error("Some error occurred while forking Exporttool! Resturncode: ".$?>>8);
		    return(false);
		}
		if ($res) {
		    my $rc = $?>>8;			
			$log->debug("Exporttool Fork with $res ended successful with returncode ".$rc);
			if($rc == 0) {
				$log->info("Exporttool finished successful");
				return(true);
			} else {
				$log->error("Exporttool failed to finish successful! Exporttool return code: ".$rc." ! Please check export tool logs under ".$exporttoollog." !");
				return(false);
			}			
		}
	}
...
# Sub to cleanup old exportool logs
		...
		while (my $file = readdir(DIR)) {
			if($file=~".log") {
				...
				if($creationtime < $deleteifolder) {
					$log->debug("Deleted exportool logfile ".$exporttoollog."/".$file);
...
# Sub to run CCI command non blocking and return output of CCI command as arrary
...
sub starthorcm {
	...
	foreach my $returnline (@returnvalue) {
		if ($returnline =~ "already been running") {
			$log->debug("Failed to start HORCM instance #",$horcminst,"! Instance allready running! Restarting instance!");
			shutdownhorcm();
			starthorcm();
...
sub makeplainldevid {
	...
	my $ldev = "";
	# Die LDEV IDs müssen im Format angepasst werden. RAIDCOM gibt "nur" ein HEX Zahl aus...
	if($ldevstringlen==1) {
	...
	foreach my $poolid (sort keys %poolstats) {
		...
		foreach my $line (@results) {
			if ($line =~ "^LDEV :") {
				...
				# Für die Zuornduning zwischen LU und LDEV werden die Port Informationen von raidcom get ldev ausgewertet und die Daten in den entsprechenden HASH geschrieben.
	...
	foreach my $port (sort keys %lumapping) {
		foreach my $hsdindex (sort keys %{$lumapping{$port}}) {
			if(defined($hsdreference{$port}{$hsdindex})) {
				...
				$log->trace("No HSD Name found for HSD-Number: ".$hsdindex." on port ".$port." for storage ".$namereference{$serial});
			}
		}
	}
	closesocket();
...
sub logscriptstats {		
        ...
        toGraphite("hds.et2graphite.stats.".$arraytype{$serial}.".".$namereference{$serial}.".".$metric." ".$value." ".$scripttime);
...
# Sub to to archive export tool data after retrieved from storage system

sub archiveexporttooldata {
	console("Archiving Exporttool-data...");	
...
sub gethourstoimport {
	...
	$log->info("Next run should import ".$hours." hours of data from Exporttool!");
	return($hours);
}
...
do {
	...
	if(!$noexporttool) {
		servicestatus("Running exporttool...");
		createexporttoolconf();
		console("Starting exporttool for serial: ".$serial.". This might take some minutes...");
		$temptime = time;
		$exporttoolstatus = startexporttool();
		logscriptstats("runtime.exporttool",time-$temptime,$starttime,true);
	}

	# Reading the metricfile...
	...
	if($exporttoolstatus) {
		...
		servicestatus("Importing exportool data...");
		$temptime = time;
		importmetric();
		...
		servicestatus("Exporttool failed... Trying next interval...");
	}
	...
	if($exporttoolstatus) {
		...
		$log->warn("Omitting archiving of exporttool data since exporttool didn't run successful. Please check exporttool and see error messages above!");
	}
